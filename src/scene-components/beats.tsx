...
interface BeatsMeta extends CommonMetaData {
...
function Beats({
    ...
    height,
    width,
}: Scene.RawComponentProps<BeatsMeta>) {
...
function LinePathRenderer({ geoGenerator, color, children, width, density }: {
    ...
    xPositions.pop();
    const positions: { x: number, y: number }[][] = [...Array(3)].map(() =>
...
const beatsModule: Scene.ComponentModule<BeatsMeta> = {
    Icon: PlannerReviewIcon,
    name:  "Beats",
    route:  "beats",
    meta: beatsMeta,
    Component: ParamsResolvingWrapper(Beats, beatsMeta),
}
